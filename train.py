...
from model import G, D,Fnet, weights_init
...
parser.add_argument('--lamda_F', type=int, default=10, help='feature loss sfactor')
...
def train(epoch):
    for (i, images) in enumerate(train_data):
        ...
        gt_f=gt.clone()
        outF=netF.extract_feature(gt_f)
        ...
        fake_B,output_f = netG(torch.cat((albedo, direct, normal, depth), 1))
        ...
        err_g = criterion(output, label) + opt.lamda * criterion_l1(fake_B, gt) + opt.lamda_F * criterion_F(output_f,outF)
        #print(opt.lamda * criterion_l1(fake_B, gt))
        #print(opt.lamda_F * criterion_F(output_f,outF))
        err_g.backward()
...
def save_checkpoint(epoch):
    ...
    for index, images in enumerate(val_data):
        ...
        normal.data.resize_(normal_cpu.size()).copy_(normal_cpu)
        depth.data.resize_(depth_cpu.size()).copy_(depth_cpu)
        out,out_f = netG(torch.cat((albedo, direct, normal, depth), 1))
        out = out.cpu()
        out_img = out.data[0]
...
print("finish training, total training time: {} seconds".format(training_time))
