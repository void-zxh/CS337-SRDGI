...
raw_path="../output/cornellboxoc/"
class_list=["Avalokitoc","cubeoc","cylinderoc","sphereoc"]
test_list=['statueoc']
root_path="../dataset/cornellboxoc/"
...
def data_split():
    ...
    for c in test_list:
        ...
        for key in image_list.keys():
            image_list[key].sort()
        list_len=len(image_list["gt"])
        for id in range(list_len):
            for p in image_path:
                dst_path=root_path+'test/'+p
                if not os.path.exists(dst_path):
                    os.makedirs(dst_path)
                shutil.copy(path+"/"+image_list[p][id], dst_path)
